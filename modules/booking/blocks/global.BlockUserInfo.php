...
if( ! nv_function_exists( 'nukevn_block_userinfo' ) )
{
	...
	{
		global $client_info, $groupUsers, $getSetting, $module_info, $global_config, $module_name, $db_slave, $user_info, $lang_global, $my_head, $admin_info, $blockID, $nv_Cache, $db, $module_info, $site_mods, $db_config;
		...
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/booking/BlockUserInfo.tpl' ) )
			{
				$block_theme = $module_info['template'];
			}
			elseif( file_exists( NV_ROOTDIR . '/themes/' . $module_info['site_theme'] . '/modules/booking/BlockUserInfo.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'BlockUserInfo.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/booking' );

			if( file_exists( NV_ROOTDIR . '/modules/booking/language/' . NV_LANG_DATA . '.php' ) )
			{
				include NV_ROOTDIR . '/modules/booking/language/' . NV_LANG_DATA . '.php';
			}
			else
			{
			...
			if( defined( 'NV_IS_USER' ) )
			{
				...
				$xtpl->assign( 'URL_statics', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=statics' );
				...
				$xtpl->assign( 'URL_PATIENT_FOLLOW', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=patientfollow' );
				$xtpl->assign( 'URL_APPOINTMENT', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=appointment' );
				...
				{
					$xtpl->parse( 'signed.doctors' );
				}
				
				if( in_array( $getSetting['default_group_doctors'], $user_info['in_groups'] ) or $user_info['group_id'] == 1 or $user_info['group_id'] == 2 or $user_info['group_id'] == 1 or $user_info['group_id'] == 3)
				{
				...
				else
				{
					...
					$xtpl->assign( 'URL_REGISTER_MEDICAL', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=register_medical');
				...
				{
					$xtpl->parse( 'signed.group_doctors_new' );
				}
