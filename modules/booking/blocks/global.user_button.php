...
function nukevn_block_userlogin( $block_config )
{
	global $site_mods, $db_config, $db, $nv_Cache, $module_info, $getSetting, $client_info, $global_config, $module_file, $module_name, $user_info, $lang_global, $my_head, $admin_info, $blockID;
	$content = '';
	...
	{
		if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/booking/block.user_button.tpl' ) )
		{
			$block_theme = $module_info['template'];
		}
		elseif( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/booking/block.user_button.tpl' ) )
		{
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/css/users.css' ) )
		{
			$block_css = $module_info['template'];
		}
		elseif( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/css/users.css' ) )
		{
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/js/users.js' ) )
		{
			$block_js = $module_info['template'];
		}
		...
		$xtpl = new XTemplate( 'block.user_button.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/booking' );

		if( $site_mods[$block_config['module']]['module_file'] != $module_file )
		{
			if( file_exists( NV_ROOTDIR . '/modules/booking/language/' . NV_LANG_INTERFACE . '.php' ) )
			{
				include NV_ROOTDIR . '/modules/booking/language/' . NV_LANG_INTERFACE . '.php';
			}
			else
			{
				include NV_ROOTDIR . '/modules/booking/language/vi.php';
			}
			if( ! empty( $block_css ) )
			{
		...
		if( defined( 'NV_IS_USER' ) )
		{
			...
			$check_user_role = $db->query('SELECT COUNT(*) FROM ' . $db_config['prefix'] . '_users t1 INNER JOIN ' . NV_PREFIXLANG . '_booking_patient t2 ON t1.userid = t2.userid WHERE t1.userid = ' . $user_info['userid'] . ' AND t2.mode = 0')->fetchColumn();

			if($check_user_role == 1){
				$info_patient = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_booking_patient WHERE userid = ' . $user_info['userid'] . ' AND mode = 0')->fetch();
				$user_info['full_name'] = $info_patient['full_name'];
				$user_info['gender'] = $info_patient['gender'];
				$user_info['birthday'] = $info_patient['birthday'];
			}
			$xtpl->assign( 'USER', $user_info );
			$xtpl->assign( 'WELCOME', defined( 'NV_IS_ADMIN' ) ? $lang_global['admin_account'] : $lang_module['welcome'] );
			...
			{
				$xtpl->assign( 'URL_CALENDAR', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=calendar' );
				$xtpl->assign( 'URL_PATIENT', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=patient' );
				
				$xtpl->parse( 'signed.group_doctors' );
			}
			else
			{
				$xtpl->assign( 'URL_HISTORY', NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='. $module .'&amp;' . NV_OP_VARIABLE . '=history' );
				
				$xtpl->parse( 'signed.group_users' );
			}
		...
		else
		{
			...
			$xtpl->assign( 'NV_REDIRECT', nv_redirect_encrypt(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=booking') );
			...
			if( $global_config['allowuserreg'] )
			{
				...
				else
				{
					...
					$_mod_data = NV_USERS_GLOBALTABLE;
...
if( defined( 'NV_SYSTEM' ) )
{
	$content = nukevn_block_userlogin( $block_config );
}
